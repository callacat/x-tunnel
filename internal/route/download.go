...
func downloadWithMirrors(ctx context.Context, rawURL string) ([]byte, error) {
	// 本地回环/本机地址（httptest、自建 GEO 服务）不经加速镜像——镜像拼接
	// 会产生外部 URL 反而绕远且失败；直接原始 URL 拉取。这也让 offline
	// 下载测试（httptest 模拟）真正离线可跑。
	if isLocalDownloadURL(rawURL) {
		return fetch(ctx, directHTTPClient, rawURL)
	}
	var lastErr error
...
// isLocalDownloadURL 判定下载 URL 是否指向本机回环地址（127.0.0.1 / ::1 /
// localhost）：这类地址是本地自建源或 httptest 测试端点，任何加速镜像都
// 不应参与——镜像拼接出的外部 URL 无法到达本机，只会白白失败/超时。
func isLocalDownloadURL(rawURL string) bool {
