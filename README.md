...
- `DNS 查询失败` or `未找到 ECH 参数`: the configured `-dns` resolver could not return HTTPS/ECH records when ECH is enabled with `-ech`. ECH is disabled by default; use standard TLS 1.3 or `-fallback`.
